...
Lors de le recherche de cycle, nous passons par les noeuds déjà marqués car, il est possible qu'il faille passer par un noeud déjà marqué pour supprimer un cycle supplémentaire (voir figure)

%Mettre le diagram whyTag.dot ici
\begin{figure}[h]
   \caption{\label{etiquette} titre}
   \includegraphics[]{whyTag.pdf}
\end{figure}

Ce qui nous donne après un premier passage (ou l'on supprime l'arrête $A \rightarrow D$ du cycle $A \rightarrow D \rightarrow E \rightarrow F \rightarrow A$) la figure %figure !%
%Mettre le diagram whyTagSimpl.dot ici
